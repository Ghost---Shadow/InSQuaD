...
def format_rank_column(method, avg_ranks, best_rank, include_rank):
...
def format_average_column(method, avg_performance, best_average, include_average):
...
def prepare_dataframe_for_latex(df, num_columns):
    """Prepare dataframe by setting oracle and hline values to 0 for max calculation."""
    df_copy = df.copy()
...
def generate_enhanced_latex_rows(
    ...
    latex_rows = ""
    max_values = prepare_dataframe_for_latex(df, num_columns)
