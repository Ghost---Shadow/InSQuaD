...
def get_checkpoint_id(key):
    if key in all_methods:
        setting = "hr_to_lr"
        method = key
    elif key in [method + "-inst" for method in all_methods] or key in [
        method + "-instruction" for method in all_methods
    ]:
...
def download_file(_id, dest):
    ...
    if _id.startswith("https://"):
        ...
        command = (
            """wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=%s' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\\1\\n/p')&id=%s" -O %s && rm -rf /tmp/cookies.txt"""
            % (_id, _id, dest)
        )
