...
class MetaICLData(object):
    ...
    def __str__(self):
        ...
        if self.metadata is None:
            ...
            text += (
                "Currently containing %d examples with %d tensors to be fed in\n"
    ...
    def evaluate(self, predictions, groundtruths, is_classification):
        ...
        for prediction, groundtruth in zip(predictions, groundtruths):
            ...
            groundtruth = (
                [gt.strip() for gt in groundtruth]
                if type(groundtruth) == list
                else groundtruth.strip()
            )
            is_correct = (
                prediction in groundtruth
                if type(groundtruth) == list
                else prediction == groundtruth
            )
    ...
    def _prepro_each_datapoint(
        ...
        if is_training or for_demonstrations:
            ...
            if "task" in dp:
                if (
                    dp["task"].startswith("inst:piqa")
                    or dp["task"].startswith("inst:yahoo_answers_topics")
                ) and len(input_tokens) + len(
                    output_tokens
                ) + 2 > self.max_length_per_example:
                    input_tokens = input_tokens[: self.max_length_per_example // 2]
                    output_tokens = output_tokens[
                        : self.max_length_per_example // 2 - 2
                    ]

                elif len(input_tokens) >= self.max_length_per_example - 2 - len(
                    output_tokens
                ):
                    if dp["task"].startswith("inst:") and len(input_tokens) < len(
                        output_tokens
                    ):
                        output_tokens = output_tokens[
                            : self.max_length_per_example - 2 - len(input_tokens)
                        ]
                    else:
            ...
            assert (
                len(input_tokens) + len(output_tokens) + 2
                <= self.max_length_per_example
            ), (
                dp.get("task", None),
                len(input_tokens),
                len(output_tokens),
            ...
            option_tokens = [
                self.tokenizer(option)["input_ids"] for option in dp["options"]
            ]
    ...
    def _tensorize_for_training(self, train_data):
        for dp in train_data:
            ...
            assert "input" in dp and "output" in dp, (
                "Training example should contain input and output",
                dp,
            )
        ...
        if self.use_demonstrations:
            ...
            for dp in train_data:
                first_tokenized.append(
                    self._prepro_each_datapoint(dp, is_first=True, is_training=True)
                )
                nonfirst_tokenized.append(
                    self._prepro_each_datapoint(dp, is_first=False, is_training=True)
                )
            ...
            def _draw_random(tot, n, exclude_indices):
                r = np.random.choice(
                    [i for i in range(tot) if i not in exclude_indices]
                )
                if n == 1:
                    return [r]
            ...
            for dp_idx, dp in enumerate(train_data):
                for _ in range(N):
                    ...
                    for demo_idx, index in enumerate(demo_indices):
                        if demo_idx == 0:
                            inputs += (
                                first_tokenized[index][0] + first_tokenized[index][1]
                            )
                        else:
                            inputs += (
                                nonfirst_tokenized[index][0]
                                + nonfirst_tokenized[index][1]
                            )
                        assert index != dp_idx
        ...
        if self.use_demonstrations:
            ...
            for dp_idx, dp in enumerate(train_data):
                for _ in range(N):
                    ...
                    # create a mapping
                    mapping = {
                        option: np.random.choice(english_words_set)
                    ...
                    for demo_idx, index in enumerate(demo_indices):
                        ...
                        curr_demo_dp["options"] = [
                            mapping[o] for o in curr_demo_dp["options"]
                        ]
                    ...
                    inputs_, outputs = self._prepro_each_datapoint(
                        curr_dp, is_first=False, is_training=True
                    )
                    inputs += inputs_
                    encoded = prepro_sentence_pair_single(
                        inputs,
    ...
    def tensorize(self, _train_data, _test_data, options=None, add_newlines=True):
        ...
        for dp in _test_data:
            ...
            assert "input" in dp and "options" in dp and type(dp["options"]) == list, (
                "Test example should contain input and options in a list format",
                dp,
            )
            if "output" not in dp:
                dp["output"] = dp["options"][
                    0
                ]  # randomly choose one (we don't need it anyways)
        ...
        for dp_idx, dp in enumerate(test_data):
            ...
            metadata.append(
                {"indices": indices, "answer": answer, "options": dp["options"]}
            )
    ...
    def tensorize_for_training(
        self, train_data, keyword, seed, use_random_english_words=False
    ):
        ...
        k_name = (
            "%d-%d" % (len(train_data), self.k)
            if self.use_demonstrations
            else len(train_data)
        )
        length_name = (
            "%d-%d" % (self.max_length, self.max_length_per_example)
            if self.use_demonstrations
            else self.max_length
        )
        postfix = "-randomEnglish" if use_random_english_words else ""

        tensorize_path = os.path.join(
            self.tensorize_dir,
            "{}_{}_k={}_seed={}_length={}{}-rank=%d.pkl".format(
                keyword, method_name, k_name, seed, length_name, postfix
            ),
        )
        ...
        if not self.do_tensorize:
            if not np.all([os.path.exists(_path) for _path in all_tensorize_paths]):
                self.logger.info(
                    "Tensorization was not done. Run with `--do_tensorize` without distributed mode"
        ...
        sharded_inputs = []
        if self.use_demonstrations or (
            len(unique_task_names) > 200 and len(train_data) >= 1638400
        ):
            tot = 0
            for i, curr_train_task in enumerate(unique_task_names):
                curr_train_data = [
                    dp for dp in train_data if dp["task"] == curr_train_task
                ]
                tot += len(curr_train_data)
                if (
                    self.use_demonstrations
                    and len(unique_task_names) > 200
                    and len(train_data) >= 1638400
                ):
                    # data is too huge; sampling 10% of the data
                    self.logger.info(
                        "Sampling training data from %d to %d",
                        len(curr_train_data),
                        len(curr_train_data) // 10,
                    )
                    indices = np.random.permutation(range(len(curr_train_data)))[
                        : len(curr_train_data) // 10
                    ]
            ...
            for i in range(self.n_process):
                sharded_inputs.append(
                    train_data[i * n_per_shard : (i + 1) * n_per_shard]
                )

        inputs = {"input_ids": [], "attention_mask": [], "token_type_ids": []}
        _tensorize_for_training = (
            self._tensorize_for_training_with_random_english_words
            if use_random_english_words
            else self._tensorize_for_training
        )
        if self.n_process == 1:
...
def prepro_sentence_pair(
    ...
    for test_input in test_inputs:
        for train_input in train_inputs:
            _input_ids, _attention_mask, _token_type_ids = prepro_sentence_pair_single(
                train_input,
                test_input,
                max_length,
                bos_token_id,
