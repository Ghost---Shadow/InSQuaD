...
from metaicl.utils import get_checkpoint_id, download_file
...
class MetaICLModel(object):
    def __init__(self, logger=None, out_dir=None, fp16=True, local_rank=-1):
        ...
        if self.local_rank <= 0:
            logger.info(
                "Setting up for local_rank=%d, world_size=%d" % (self.local_rank, ws)
            )
    ...
    def __str__(self):
        ...
        text += "\nusing device %s, %d gpus, local_rank=%d" % (
            self.device,
            self.n_gpu,
            self.local_rank,
        )
    ...
    def setup_optimizer(
        ...
        if optimization == "adafactor":
            ...
            optimizer = bnb.optim.Adam8bit(
                optimizer_grouped_parameters, lr=lr, betas=(0.9, 0.995)
            )
            if self.fp16:
                self.model, optimizer = setup_fp16(self.model, optimizer)
            scheduler = get_linear_schedule_with_warmup(
    ...
    def do_train(
        ...
        n_trainable_params = len(
            [param for param in self.model.parameters() if param.requires_grad]
        )
        n_gpus = torch.cuda.device_count()
        self.logger.info(
            "Training {} parameters on {} examples for {} steps using {} GPUs".format(
    ...
    def do_predict(self, data, batch_size=1, losses=None, verbose=False):
        ...
        for idx, dp in enumerate(data.metadata):
            ...
            prediction_idx = sorted(enumerate(curr_label_losses), key=lambda x: x[1])[
                0
            ][0]
