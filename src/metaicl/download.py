...
from metaicl.utils import all_settings, all_methods
from metaicl.utils import download_file, get_checkpoint_id
...
def parse_args():
    ...
    parser.add_argument(
        "--setting", default="all", type=str, choices=["all"] + all_settings
    )
    parser.add_argument(
        "--method", default="all", type=str, choices=["all"] + all_methods
    )
...
def main(args):
    if args.demo_data:
        download_file(
            "15grQwt3B1tALtUCGtaDI_rwC28LL8wSj",
            os.path.join(
                args.data_dir,
                "financial_phrasebank",
                "financial_phrasebank_16_100_train.jsonl",
            ),
        )
    ...
    if args.checkpoints:
        ...
        for method in methods:
            for setting in settings:
                ...
                download_file(
                    _id, os.path.join(args.checkpoint_dir, method, setting, "model.pt")
                )
